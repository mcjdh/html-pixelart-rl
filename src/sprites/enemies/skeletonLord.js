const skeletonLordSprites = {
    default: function(ctx, x, y, size) {
        ...
        // Menacing crown/horn protrusions
        ...
        // Ancient weapon in left hand (sword hilt)
        ctx.fillStyle = '#8b4513'; // Brown handle
        ctx.fillRect(x + 1*unit, y + 11*unit, 1*unit, 3*unit);
        ctx.fillStyle = '#c0c0c0'; // Metal blade
        ctx.fillRect(x + 1*unit, y + 8*unit, 1*unit, 4*unit);
        
        // Shield in right hand (moved within bounds)
        ctx.fillStyle = '#654321'; // Dark wood
        ctx.fillRect(x + 13*unit, y + 10*unit, 2*unit, 4*unit);
        ctx.fillStyle = '#888'; // Metal rim
        ctx.fillRect(x + 13*unit, y + 10*unit, 2*unit, 1*unit);
        ctx.fillRect(x + 13*unit, y + 13*unit, 2*unit, 1*unit);
        ...
        // Dark energy crackling around the boss
        if (darkPulse > 0.8) {
            ctx.fillStyle = 'rgba(160, 0, 160, 0.7)';
            // Energy sparks at random positions
            const spark1X = x + Math.floor((4 + Math.sin(time * 3) * 3) * unit);
            const spark1Y = y + Math.floor((6 + Math.cos(time * 2.7) * 2) * unit);
            ctx.fillRect(spark1X, spark1Y, 1*unit, 1*unit);
            
            const spark2X = x + Math.floor((10 + Math.sin(time * 2.3) * 3) * unit);
